...
# ────────────────────────────────────────────────────────────
# The same codebase runs in two places with very different budgets:
#
#   local   — no memory ceiling worth worrying about, so the heavier
#             local models (torch cross-encoder re-ranker) are enabled.
#   render  — 512MB RAM on the free plan. Only the lightweight
#             fastembed/onnxruntime embedder fits; torch does not.
#
# Render sets RENDER=true in every service, so detection is automatic.
# APP_ENV overrides it explicitly when you want to simulate one or the other.
IS_RENDER = os.environ.get("RENDER", "").lower() in ("true", "1", "yes")
APP_ENV = os.environ.get("APP_ENV", "render" if IS_RENDER else "local").strip().lower()
IS_LOCAL = APP_ENV == "local"


def _env_flag(name: str, default: bool) -> bool:
    """Read a boolean env var, falling back to an environment-aware default."""
...
SEMANTIC_WEIGHT = 1.0 - BM25_WEIGHT  # Weight of dense/semantic score in hybrid search
...
# Cross-encoder re-ranking is the single heaviest optional feature: it pulls in
# sentence-transformers + torch, which need ~500MB+ of RAM on their own. That
# fits comfortably on a local machine and not at all on Render's 512MB free
# plan, so the default follows the environment. CROSS_ENCODER_RERANK overrides
# it either way; if the package is missing, embeddings_utils degrades to the
# metadata re-ranker instead of failing.
CROSS_ENCODER_RERANK = _env_flag("CROSS_ENCODER_RERANK", IS_LOCAL)
